...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === 'bill') {
      const bill = db.prepare('SELECT * FROM ap_bills WHERE id = ?').get(id) as any;
      if (!bill) return NextResponse.json({ error: 'Bill not found' }, { status: 404 });

      db.prepare(`UPDATE ap_bills SET status = 'voided', updated_at = datetime('now') WHERE id = ?`).run(id);

      // Create reversing journal entry if original had one
      if (bill.journal_entry_id) {
        const origJE = db.prepare('SELECT * FROM journal_entries WHERE id = ?').get(bill.journal_entry_id) as any;
        if (origJE) {
          ...
          const je = db.prepare(`INSERT INTO journal_entries (entry_number, entry_date, description, reference_type, total_debit, total_credit, status, created_by) VALUES (?, date('now'), ?, 'reversal', ?, ?, 'posted', 3)`)
            .run(jeNum, `Void: ${bill.bill_number} - ${bill.description || ''}`, origJE.total_debit, origJE.total_credit);

          // Reverse all lines (swap debit/credit)
          const lines = db.prepare('SELECT * FROM journal_entry_lines WHERE journal_entry_id = ?').all(bill.journal_entry_id) as any[];
      ...
      return NextResponse.json({ success: true, message: `Bill ${bill.bill_number} voided` });
    }
