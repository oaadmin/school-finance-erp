...
export default function APAgingReport() {
  ...
  useEffect(() => {
    fetch('/api/accounting/ap?type=aging').then(r => r.json()).then(setRows);
  }, []);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Accounts payable aging summary &middot; Total Outstanding: {formatCurrency(totals.total)}
        </p>
      </div>
      ...
      <div className="card">
        <div className="card-header flex items-center gap-2">
          ...
          <h3 className="font-semibold text-gray-900">Aging Detail by Vendor</h3>
        </div>
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.vendor_id}>
                  <td className="font-medium text-xs sm:text-sm">{r.vendor_name}</td>
